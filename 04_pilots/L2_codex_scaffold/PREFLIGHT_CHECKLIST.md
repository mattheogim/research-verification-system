# Codex L2 Preflight Checklist

Codex L2 실행 (Session C) 전에 **모든 항목** 확인.
하나라도 미완료면 실행 금지.
...
- [ ] Tag에 포함된 파일:
  ...
  - [ ] `codex_L2_prompt_v1.md` (placeholder 채워진 버전)
  - [ ] `claude_L2_prompt_v1.md` (placeholder 채워진 버전)
  - [ ] `L2_input.csv`

## Prompt 완성 확인

- [ ] `codex_L2_prompt_v1.md`에 `PLACEHOLDER` 문자열이 **0개**
  ```bash
  grep -c 'PLACEHOLDER' L2_codex_scaffold/codex_L2_prompt_v1.md
  # 결과가 0이어야 함
  ```
...
- [ ] Claude prompt와 Codex prompt가 output 파일명 외 동일
  ```bash
  diff <(sed 's/codex_L2_results/OUTPUT/g' L2_codex_scaffold/codex_L2_prompt_v1.md) \
...
- [ ] Session B와 Session C가 서로의 결과를 모름
...
모든 항목 확인 후:
1. 새 Codex 세션 열기
2. `codex_L2_prompt_v1.md` 전문을 프롬프트로 전달
3. `L2_input.csv`를 세션에 제공
4. Codex가 `codex_L2_results.csv` 생성하면 이 디렉토리에 저장
